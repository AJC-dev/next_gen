...
function drawPreviewCanvas() {
    ...
    ctx.lineWidth = 1;
    ctx.setLineDash([5, 5]);
    const safetyMarginX = (postcardConfig.settings.bleedMM / postcardConfig.settings.a5WidthMM) * canvas.width;
    const safetyMarginY = (postcardConfig.settings.bleedMM / postcardConfig.settings.a5HeightMM) * canvas.height;
    ...
    if (appState.frontText.text) {
        ...
        Object.values(handles).forEach(handle => {
            ctx.beginPath();
            ctx.arc(handle.x, handle.y, postcardConfig.settings.handleRadius, 0, 2 * Math.PI);
            ctx.fill();
            ctx.stroke();
        });
    }
...
async function validateAndSetImage(src) {
    ...
    if (tempImage.width < postcardConfig.settings.minImageDimension || tempImage.height < postcardConfig.settings.minImageDimension) {
        dom.imageWarning.textContent = `For best quality, please upload an image that is at least ${postcardConfig.settings.minImageDimension}px on its shortest side.`;
...
async function checkMessageOverflow() {
    ...
    const finalWidthPx = Math.round((postcardConfig.settings.a5WidthMM / 25.4) * postcardConfig.settings.printDPI);
    ...
    const maxTextHeight = Math.round((postcardConfig.settings.a5HeightMM / 25.4) * postcardConfig.settings.printDPI) - (hiResFontSize * 1.2) - 50;
...
async function generatePostcardImages({ forEmail = false } = {}) {
    ...
    if (forEmail) {
        ...
        previewCanvas.height = mainContentHeightPx;
        ...
        if (appState.uploadedImage) {
            ...
            if (appState.isPortrait) {
                tempCanvas.width = dom.previewCanvas.el.height;
                tempCanvas.height = dom.previewCanvas.el.width;
            } else {
                tempCanvas.width = dom.previewCanvas.el.width;
                tempCanvas.height = dom.previewCanvas.el.height;
            }
            ...
            if (appState.isPortrait) {
                ...
                previewCtx.drawImage(tempCanvas, -previewCanvas.height / 2, -previewCanvas.width / 2, previewCanvas.height, previewCanvas.width);
                previewCtx.restore();
            } else {
        ...
        printCanvas.height = (appState.isPortrait ? mainContentWidthPx : mainContentHeightPx) + (bleedPx * 2);
...
async function initialize() {
    ...
    dom.imageUploader.addEventListener('change', async (event) => {
        ...
        reader.onload = async (e) => {
            ...
            if (file.size > postcardConfig.settings.maxFileSizeMB * 1024 * 1024) {
    ...
    const handleInteractionStart = (e) => {
        ...
        if (appState.frontText.text) {
            ...
            if (metrics && metrics.x !== null) {
                ...
                if (distToResize <= postcardConfig.settings.handleRadius) {
                    interactionMode = 'resizing';
                    isOverText = true;
                } else if (distToRotate <= postcardConfig.settings.handleRadius) {
                    interactionMode = 'rotating';
                    isOverText = true;
                } else if (distToWidth <= postcardConfig.settings.handleRadius) {
